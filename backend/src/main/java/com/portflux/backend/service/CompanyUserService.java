...
public class CompanyUserService {
    ...
    public boolean registerCompany(CompanyRegisterBean registerBean) {
        ...
        if (companyUserMapper.checkBusinessNumber(registerBean.getBusinessNumber()) > 0) {
            throw new RuntimeException("이미 등록된 사업자번호입니다.");
        }

        // 사업자번호 유효성 검사는 프론트엔드에서 이미 수행되었으므로 생략
        // (공공데이터 API Rate Limiting 문제 방지)
