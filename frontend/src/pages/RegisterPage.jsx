...
function RegisterPage() {
  ...
  useEffect(() => {
    ...
    const timer = setTimeout(async () => {
      ...
      } catch (e) { console.error("이메일 중복 확인 에러", e); }
    }, 500);
  ...
  const handleSendAuthCode = async () => {
    if (isEmailValid === false) { alert(emailMsg || "이메일을 확인해 주세요."); return; }
    if (isEmailValid === null) { alert("이메일 중복 확인 중입니다."); return; } 
    try {
      const res = await fetch("http://localhost:8080/api/mail/send", {
        method: "POST", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ email: email }),
      });
      if (res.ok) { alert(await res.text()); setAuthCode(""); setIsAuthVerified(false); }
  ...
  const handleVerifyAuthCode = async () => {
    ...
    try {
      ...
      if (res.ok) {
        if (await res.json()) { alert("이메일 인증 성공!"); setIsAuthVerified(true); }
        else { alert("인증코드가 일치하지 않습니다."); setIsAuthVerified(false); }
      } else alert("인증 확인 오류");
    } catch { alert("서버 연결 실패"); }
  };

  const handleRegister = async () => {
    if (!userId || !email || !authCode || !password || !passwordCheck || !userName || !nickname || !phoneNumber) {
      alert("모든 정보를 입력해 주세요."); return;
    }
    if (isIdAvailable !== true) { alert("아이디 중복 확인을 해주세요."); return; } 
    if (isEmailValid !== true) { alert("이메일을 확인해 주세요."); return; }
    ...
    if (!isIndividual && isBizNumValid !== true) { alert("올바른 사업자등록번호를 입력하고 확인해 주세요."); return; }
    ...
    try {
      ...
      if (res.ok) { 
        ...
        navigate("/login"); 
      } else { alert("가입 실패: " + await res.text()); }
    } catch { alert("서버 연결 실패"); }
  };
      ...
      <div className="register-box">
        ...
        <div className="form-container">
          ...
          <div className="form-group">
            ...
            <div className="input-with-btn">
              <input type="text" className={`reg-input ${isEmailValid === false ? "input-error" : ""}`} 
                     placeholder="이메일을 입력해 주세요." value={email} onChange={handleEmailChange} disabled={isAuthVerified} />
              <button className="btn-small" onClick={handleSendAuthCode} disabled={isAuthVerified}>인증코드 발송</button>
            </div>
            {isEmailValid === false && <span className="error-msg">{emailMsg}</span>}
            {isEmailValid === true && <span className="valid-msg">{emailMsg}</span>}
            
            <div className="input-with-btn">
              <input type="text" className="reg-input" placeholder="인증코드 6자리" value={authCode} onChange={(e) => setAuthCode(e.target.value)} disabled={isAuthVerified} />
          ...
          <div className="form-group">
            ...
            <input type="text" className="reg-input" placeholder="번호를 입력해 주세요." value={phoneNumber} onChange={handlePhoneNumberChange} maxLength={13} />
          </div>
